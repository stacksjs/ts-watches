...
const WAHOO_VOLUME_PATTERNS = ['WAHOO', 'ELEMNT', 'BOLT', 'ROAM', 'RIVAL']

export interface WahooDevice extends WatchDevice {
  type: 'wahoo'
}

export class WahooDriver implements WatchDriver {
  readonly name = 'Wahoo'
  readonly type = 'wahoo' as const

  async detectDevices(): Promise<WahooDevice[]> {
    const devices: WahooDevice[] = []
    ...
    for (const volume of volumes) {
      ...
      const isWahoo = WAHOO_VOLUME_PATTERNS.some(p => upperVolume.includes(p))

      // Also check for Wahoo folder structure
      if (!isWahoo) {
        const wahooFolder = join(volumePath, 'activities')
        if (existsSync(wahooFolder)) {
          // Check if it looks like a Wahoo device
          const files = readdirSync(wahooFolder)
          if (files.some(f => f.toLowerCase().endsWith('.fit'))) {
            devices.push({
              name: volume,
              path: volumePath,
              type: 'wahoo',
  ...
  async downloadData(device: WatchDevice, options: DownloadOptions = {}): Promise<DownloadResult> {
    ...
    // Wahoo stores activities in FIT format in various locations
    const activityDirs = [
      join(device.path, 'activities'),
      join(device.path, 'Activity'),
      join(device.path, 'workouts'),
    ]
      ...
      for (const activityDir of activityDirs) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (copyRawFiles) {
              const destDir = join(downloadPath, 'activities')
...
export function createWahooDriver(): WahooDriver {
  return new WahooDriver()
}
